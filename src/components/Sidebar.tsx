...
export function Sidebar({ active, onSelect, currentUserData }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar móvil */}
      <div
        className="fixed top-0 left-0 z-50 h-full w-64 transform -translate-x-full transition-transform duration-300 bg-panel border-r border-border lg:hidden"
        id="mobile-sidebar"
      >
        <div className="p-6">
          <div className="mb-8">
      ...
      <button
        ...
        onClick={() => {
          ...
          if (overlay) { overlay.classList.toggle('opacity-0'); overlay.classList.toggle('pointer-events-none'); }
        }}
      >
